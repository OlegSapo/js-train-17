...
function replaceText(word, replacement, text) {
  ...
  console.log(text.replaceAll(regex, replacement));
  // return result;
}
...
function extractTextInParentheses(str) {
  ...
  let resultMod = [];
  resultMod.push(result[0].slice(1, -1));
  resultMod.push(result[1].slice(1, -1));
  return resultMod;
}
...
function checkRegexFlags(regex) {
  ...
  let flgs = regex.flags;
  if (flgs.includes("g") && flgs.includes("m")) {
    return true;
  } else {
    return false;
  }
...
function checkFlags(regex) {
  ...
  let arr = [];
  let flgs = regex.flags;
  // if (regex.ignoreCase) arr.push("ignoreCase");
  regex.ignoreCase && arr.push("ignoreCase");
...
function checkRegexMethods(regex) {
  ...
  let arr = [];
  regex.dotAll && arr.push("dotAll");
  regex.multiline && arr.push("multiline");
  regex.sticky && arr.push("sticky");
  return arr;
}
